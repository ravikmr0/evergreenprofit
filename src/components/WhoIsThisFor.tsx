      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {personas.map((persona, index) => (
            <div
              key={persona.title}
              className={`section-animate group text-center p-8 rounded-2xl border ${persona.border} bg-white/[0.02] hover:bg-white/[0.05] transition-all duration-500 hover:-translate-y-2`}
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              <div className={`w-16 h-16 rounded-2xl ${persona.bg} flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300`}>
                <persona.icon className={`w-8 h-8 ${persona.color}`} />
              </div>
              <h3 className="text-lg font-bold text-white mb-3">{persona.title}</h3>
              <p className="text-sm text-white/45 leading-relaxed mb-4">{persona.description}</p>
